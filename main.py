...
def _forecast_with_lstm(df: pd.DataFrame) -> Tuple[List[float], str]:
    ...
    if _TF_AVAILABLE and _LSTM_MODEL is not None:
        ...
        # LSTM 輸出是 restore 後的「原空間」，假設訓練程式保存的是未縮放的序列；
        # 若你當初訓練是「縮放後預測、輸出未反縮放」，請把這裡替換為 inverse_transform。
...
def get_app_data():
    ...
    if feats is None:
        ...
        return {
            ...
            "risk_level": "資料不足",
            "forecast": None if forecast_vals is None else {
